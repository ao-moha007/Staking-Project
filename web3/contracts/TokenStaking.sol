...
contract TokenStaking is Ownable,ReentrancyGuard,Initializable {
    ...
    uint256 _earlyUnistakeFeePercentage;//early unstake fee percentage
    ...
    function __TokenStaking_init_unchained(
        ...
        require(tokenAddress_ != address(0),"TokenStaking: token address can't be a 0 address");
        ...
        _earlyUnistakeFeePercentage = earlyUnstakeFeePercentage_;
    ...
    *@notice This function is used to get early unstake fee percentage
     */
     function getEarlyUnstakeFeePercentage() external view returns(uint256){
        return _earlyUnistakeFeePercentage;
     }
    ...
    *@notice This function is used to update early stake fee percentage
     */
     function updateEarlyStakeFeePercentage(uint256 newPercentage) external onlyOwner{
        
        _earlyUnistakeFeePercentage = newPercentage ;
     ...
     */
     ...
     function stake(uint256 _amount) external nonReentrant {
        require(this.getwithdrawableAmount()>= _amount, "TokenStaking: not enough withdrawable tokens");
     ...
     function _stakeTokens(uint256 _amount, address user_) private{
        ...
        require(currentTime > _stakeStartDate,"TokensStaking: staking not started yet");
        require(currentTime < _stakeEndDate,"TokensStaking: staking ended");
        require(_totalStakedTokens + _amount <= _maxStakingTokenLimit,"TokensStaking: max staking token limit reached");
        require(_amount >0 , "TokenStaking: stake amount must be non-zero");
        require(_amount >= _minimumStakingAmount,"TokenStaking: stake amount must be greater than the minimum amount allowed");
    ...
    */
     function unStake(uint256 _amount) external nonReentrant whenTreasuryHasBalance(_amount){
            ...
            if(getCurrentTime() <= _users[user].lastStakeTime + _stakeDays){
                 feeEarlyUnstake = ((_amount * _earlyUnistakeFeePercentage)/ PERCENTAGE_DEMONITION);
    ...
    */
    function _getUserEstimatedRewards(address _user) private view returns (uint256, uint256) {
            ...
            userReward += ((totalStakedTime * _users[_user].stakeAmount * _apyRate) / 365 days ) / PERCENTAGE_DEMONITION;
